...
function StatusBadge({ doc }: { doc: UploadedDocument }) {
  ...
  if (doc.extractionStatus === "PROCESSING") {
    return (
      <span
        role="status"
        className="inline-flex items-center gap-2 text-sm text-live"
      >
...
export function MaterialsStep({
  ...
  async function upload(files: FileList | File[]) {
    ...
    try {
      ...
      if (!payload.ok) {
        ...
        setErrors(
          (payload.failures ?? []).map(
            (f: { filename: string; message: string }) =>
      ...
      setErrors([
        "Your document couldn't be uploaded. Check your connection and try again.",
      ]);
    } finally {
      setBusy(false);
  ...
  return (
    <div className="space-y-6">
      <StepIntro>
        Do you already have materials for this project? Add supervisor
        instructions, department guidelines, existing chapters, proposals,
        questionnaires, papers or notes. Their text is read and kept with your
        project so your own materials inform the work.
      </StepIntro>
      ...
      <div
        ...
        className={`rounded-xl border p-8 text-center transition-[border-color,background-color,border-style] duration-200 ${
          dragging
      ...
      {errors.length > 0 ? (
        <div
          role="alert"
          className="space-y-1 rounded-md border border-destructive/35 bg-destructive-subtle p-3 text-sm text-destructive"
        >
          {errors.map((e) => (
            <p key={e}>{e}</p>
          ))}
      ...
      <section>
        ...
        {documents.length === 0 ? (
          <p className="mt-2 leading-relaxed text-muted-foreground">
            Nothing uploaded yet. This step is optional — you can skip it and
            come back.
          </p>
        ) : (
              ...
              <li key={doc.id} className="flex flex-wrap items-start gap-3 p-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  */}
                  ...
                  {doc.extractionStatus === "FAILED" ? (
                    <p className="mt-1 text-sm text-muted-foreground">
                      The file is saved with your project, but its text
                      couldn&apos;t be read. Try re-saving it as a PDF or Word
                      file and uploading again.
                    </p>
                  ) : null}
                </div>
                <div className="flex gap-2">
                  <Button asChild size="sm" variant="outline">
                    <a
                      href={`/api/projects/${projectId}/documents/${doc.id}/raw`}
                    >
